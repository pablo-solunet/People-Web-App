...
export function ManagerView({ hasActionPermission }: ManagerViewProps) {
  ...
  useEffect(() => {
    filterAndSearchData()
  }, [managerData, searchTerm, filterField, filterValue]) //Corrected useEffect dependency
  ...
  const filterAndSearchData = () => {
    ...
    if (filterField && filterValue) {
      result = result.filter(
        (record) => record[filterField as keyof ManagerData]?.toString().toLowerCase() === filterValue.toLowerCase(),
      )
    }
  ...
  const updateRecordState = async (id_reg: string, newState: string, isLote: boolean, lote_id?: string) => {
    try {
      ...
      if (newState == "Aprobado") {
        ;(newestado = "Pendiente"), (newarea = "People")
      } else {
      ...
      <Card className="border-blue-100 dark:border-blue-800 shadow-blue-sm dark:shadow-none">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-medium flex items-center justify-between">
            ...
            <Button onClick={fetchManagerData} disabled={isLoading} variant="outline" size="sm" className="ml-auto">
